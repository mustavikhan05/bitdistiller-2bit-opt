...
def pseudo_quantize_model_weight(
    ...
    if quant_type == "int":
        if hasattr(model.model, "decoder"):
            layers = model.model.decoder.layers  # For OPT models that use `decoder.layers`
        elif hasattr(model, "transformer"):
            layers = model.transformer.h  # For models using `transformer.h`, e.g., some GPT-style models
        else:
            layers = model.model.layers  # Default layer access
        ...
        if hasattr(model.model, "decoder"):
            layers = model.model.decoder.layers  # For OPT models
        elif hasattr(model, "transformer"):
            layers = model.transformer.h  # For models using transformer.h
        else:
            layers = model.model.layers  # Default
        for i in tqdm(range(len(layers)), desc=f"pseudo {quant_type} weight quantization..."):
            named_linears = get_named_linears(layers[i])
            for n, m in named_linears.items():
